...
//
// The Doover platform serves a widget as a SINGLE channel attachment, so the
// several JS chunks Module Federation emits must be flattened into one file.
// This rspack/webpack plugin hooks `afterEmit`, concatenates every emitted
// `.js` chunk under `source` (except `ignore`d ones, sorted for determinism),
// and writes the result to `destination/name`
// (= ./assets/DataReportSegmenterWidget.js). Mirrors petronash-hmi's plugin.
        ...
        try {
          ...
          const contents = jsFiles
            .map((file) => {
              ...
              console.log(
                `[ConcatenatePlugin] Adding ${path.relative(sourceDir, file)}`,
              );
              return content;
            })
            .join("\n");
          ...
          console.log(
            `[ConcatenatePlugin] Created ${outputPath} (${(
