...
export function GenerateReportPanel({
  ...
  const [startValue, setStartValue] = useState<string>(initial.startValue);
  const [endValue, setEndValue] = useState<string>(initial.endValue);
  ...
  return (
    ...
    >
      ...
      {active !== null && (
        <ReportStatusLine tokens={tokens} active={active} />
      )}
    </div>
  );
}
